...
class SnakeScoreManager extends BaseScoreManager {
  constructor(difficulty) {
    super(difficulty, "snake"); // gameKey = 'snake'
  ...
  getGameInterval() {
    return 1000 / this.tps; // interval dalam ms untuk loop BaseGame
  }
}
...
class Renderer {
  ...
  drawSnake(snake) {
    snake.segments.forEach((seg, i) => {
      this.ctx.fillStyle =
        i === 0 ? Config.COLORS.snakeHead : Config.COLORS.snakeBody;
      this.ctx.beginPath();
      this.roundRect(
        seg.x * Config.TILE + 2,
        seg.y * Config.TILE + 2,
...
class SnakeGame extends BaseGame {
  constructor() {
    super("snake"); // panggil constructor BaseGame
    this.initDOM();
